...
int value[100];                // variable to store the IR values
float voltage;                 // variable that will tell if there is an obstacle or not
...
void loop(){
  voltage = readIR(50);       // calling the function that will read the distance and passing the "accuracy" to it
  Serial.println(voltage);   // writing the read value on Serial monitor
...
int readIR(int times){
  ...
  return(voltage/times);            // return the final value
}
